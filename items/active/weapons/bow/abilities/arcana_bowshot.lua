...
function BowShot:fire()
  ...
  local params = self:currentProjectileParameters()
  ...
  local powerCritBoost = self.weapon.powerCritBoost or self.powerCritBoost or {0,0}
  if self:perfectTiming() then
    critRateStat = critRateStat + powerCritBoost[1]
    critDamageMultiplier = critDamageMultiplier + powerCritBoost[2]
  end
